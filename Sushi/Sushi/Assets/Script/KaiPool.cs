...
public class KaiPool : ObjectPool
{
    public GameObject Kai; //Type of gameObject for this pool
   
    public static KaiPool instance; //holds singleton reference
    ...
    {
        return Instantiate<GameObject>(Kai);
    }
    ...
    public GameObject GetBullet()
    {
        if(pool.Count == 0){ //if there's nothing in the pool
            ...
            GameObject kai = Get(); //get a bullet out
   
             //kai.GetComponent<SashimiInformation>().Reset(); //reset it
   
            return Kai; //return it
        }
    } 
}
